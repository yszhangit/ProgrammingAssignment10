...
```{r sumplot, echo=FALSE, fig.dim = c(12, 8) }
#ggpairs(mtcars)
```
...
```{r multireg, echo=F, fig.dim = c(6,6) }
mod.mul <- lm(mpg ~ wt + qsec + am, data = mtcars)
summary(mod.mul)
```
Interpretation of the output:
This model tells us on every tone increase, the veichile fuel economy will reduce by 3.9165 mpg if other two variables remain unchanged. This is expceted as the heavier the car, the wrose the fuel economy. 
...
```{r residualcomp, echo = F, fig.dim = c(8, 4) }
...
```{r regmulplot, echo = F, fig.dim = c(8, 4) }
# init grid size
grid.lines = 20
...
scatter3D(points.auto$wt, points.auto$qsec, points.auto$mpg, 
          zlim = c(0,40), col = "blue",
          colkey=F, pch=20, cex=2, alpha = 0.5,
          phi = -5, theta = 10, bty = "g", 
